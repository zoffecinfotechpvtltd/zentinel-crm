...
type Client = { id: string; company: string; gstin: string | null; tally_ledger_name: string | null; status: string };
  ...
  return (
    <div>
      <div className="section-header">
        ...
        {canEdit && <button className="btn btn-primary" onClick={() => setAddOpen(true)}>+ Add Client</button>}
      </div>
      ...
      <div className="card" style={{ padding: 0 }}>
        <div className="table-wrap">
          <table>
            <thead><tr><th>Company</th><th>GSTIN</th><th>Tally Ledger</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
              {loading && <tr><td colSpan={5}><div className="empty">Loading…</div></td></tr>}
              {!loading && data?.data.length === 0 && <tr><td colSpan={5}><div className="empty">No clients found</div></td></tr>}
              {data?.data.map((c) => (
                <tr key={c.id}>
                  <td style={{ fontWeight: 500, color: "var(--text)" }}>{c.company}</td>
                  <td className="mono" style={{ fontSize: 12 }}>{c.gstin ?? "—"}</td>
                  <td style={{ fontSize: 12 }}>{c.tally_ledger_name ?? <span style={{ color: "var(--warning)" }}>not set</span>}</td>
                  <td><Badge status={c.status} /></td>
                  <td><button className="btn btn-ghost btn-sm" onClick={() => setDetailId(c.id)}>View</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {addOpen && (
        <Modal title="Add New Client" onClose={() => setAddOpen(false)} footer={<>
          ...
          <button className="btn btn-primary" onClick={createClient}>Save Client</button>
        </>}>
      ...
      {detailId && detail && (
        <Modal title={detail.company} onClose={() => setDetailId(null)} wide footer={<button className="btn btn-ghost" onClick={() => setDetailId(null)}>Close</button>}>
          ...
          {canEdit && (
            <div className="form-group full" style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button className="btn btn-ghost btn-sm" onClick={saveLedger}>Save</button>
              </div>
            </div>
          )}
              ...
              <tbody>
                {detail.contacts.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td>{!c.is_primary && canEdit && <button className="btn btn-ghost btn-sm" onClick={() => setPrimary(c.id)}>Make primary</button>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {canEdit && (
            <div className="filter-bar" style={{ marginBottom: 20 }}>
              ...
              <button className="btn btn-ghost btn-sm" onClick={addContact} disabled={!contactForm.name}>+ Add Contact</button>
            </div>
          )}
          ...
          {canEdit && (
            <div className="filter-bar">
              ...
              <button className="btn btn-ghost btn-sm" onClick={addContract}>+ Add Contract</button>
            </div>
          )}
        </Modal>
      )}
